...
nx_nodes = 10  # number of x- nodes
ny_nodes = 10  # number of y- nodes

dx = W/nx_nodes  # node's control volume width
dy = H/ny_nodes  # node's control volume height

# Initial condition
time_temp_dist = np.empty((n_time_steps+1, nx_nodes, ny_nodes))
...
def solve():
    ...
    while (t < TIME_INTERVAL):
        t += dt  # Change time to current time
        ...
        T[-1, 1:-1] = (a_E*T_prev[-1, 2:] + a_W*T_prev[-1, 0:-2] +
                       a_N*T_prev[-1, 1:-1] + (a_Po - (a_E + a_W + a_N))*T_prev[0, 1:-1] + S_u)/a_P
...
def animate(k):
    plotheatmap(time_temp_dist[k], k)


solve()

anim = animation.FuncAnimation(
    plt.figure(), animate, interval=1, frames=TIME_INTERVAL + 1, repeat=False)
